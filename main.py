...
from services.calculate import nearest_neighbors_vrptw, local_search, routes_length
...
if __name__ == '__main__':
    ...
    first_solutiom = copy.deepcopy(nn_solution)

    for i in range(0, length-1):
        for j in range(i + 1, length):
            if nn_solution["routes"][i] != [0, 0] and nn_solution["routes"][j] != [0, 0]:
    ...
    for route in nn_solution["routes"]:
        if route == [0, 0]:
            nn_solution["routes"].remove(route)

    nn_solution["vehicles"] = len(nn_solution["routes"])
    nn_solution["length"] = routes_length(vrptw, nn_solution["routes"])

    print(first_solutiom)
    print(nn_solution)
